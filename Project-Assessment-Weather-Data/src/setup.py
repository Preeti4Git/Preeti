...
USER_DEVICE_ACCESS_COLLECTION = 'user_device_access'
...
# user device access data import
# User document contains username (String), email (String), and role (String) fields
# Reads users.csv one line at a time, splits them into the data fields and inserts
with open(RELATIVE_CONFIG_PATH + USER_DEVICE_ACCESS_COLLECTION + '.csv', 'r') as user_fh:
    for user_access_row in user_fh:
        user_access_row = user_access_row.rstrip()
        device_access = []
        if user_access_row:
            (username, access) = user_access_row.split(',')
            for access in access.split(';'):
                (device_id, access_type) = access.split(':')
                device_access.append({'device_id': device_id, 'access_type': access_type})

        user_access_data = {'username': username, 'access': device_access}

        # This creates and return a pointer to the users collection
        user_access_collection = weather_dbh[USER_DEVICE_ACCESS_COLLECTION]

        # This inserts the data item as a document in the user collection
        user_access_collection.insert_one(user_access_data)